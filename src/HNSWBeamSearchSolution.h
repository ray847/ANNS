...
class Solution {
 public:
  ...
  Solution() {
    ...
    layer_times_ns_.resize(kMaxLevel + 1, 0);
  }

  ~Solution() {
    if constexpr (global::kDEBUG) {
      if (total_searches_ > 0) {
        std::cout << "\n=== HNSW Profiling Stats ===\n";
        ...
        for (int l = kMaxLevel; l >= 0; --l) {
            // Only print layers that actually accrued time
            if (layer_times_ns_[l] > 0) {
                double avg_us = (double)layer_times_ns_[l] / total_searches_ / 1000.0;
                std::cout << "Layer " << l << ": " << avg_us << " us\n";
                total_time += layer_times_ns_[l];
            }
        }
        std::cout << "Total Avg Latency: " << (double)total_time / total_searches_ / 1000.0 << " us\n";
        std::cout << "============================\n";
      }
    }
  }
  ...
  void build(int d, const std::vector<float>& base) {
    ...
    if constexpr (global::kDEBUG) {
      std::cout << "Building HNSW Beam Search (d=" << d_ << ", M=" << kM
                << ") for " << n_ << " vectors...\n";
    }
  ...
  void search(const std::vector<float>& query, int* res) {
    ...
    // We gradually increase 'ef' as we descend. 
    // Logic: ef = max(8, kEfSearch >> level).
    // Example (kEfSearch=300):
    // Level 4: ef = 18
    // Level 3: ef = 37
    // Level 2: ef = 75
    // Level 1: ef = 150
    ...
    for (int level = m_level; level > 0; --level) {
      ...
      auto top_candidates = BeamSearch(q_data, curr_ep, dynamic_ef, level, visited_list_);
      
      // Find closest for next layer entry
      ...
      if constexpr (global::kDEBUG) {
        layer_times_ns_[level] += std::chrono::duration_cast<std::chrono::nanoseconds>(t_end - t_start).count();
      }
    }

    // --- Layer 0 Search (Fine-Grained) ---
    {
      auto t_start = std::chrono::high_resolution_clock::now();
      ...
      auto t_end = std::chrono::high_resolution_clock::now();
      if constexpr (global::kDEBUG) {
        layer_times_ns_[0] += std::chrono::duration_cast<std::chrono::nanoseconds>(t_end - t_start).count();
      }
 ...
 private:
  ...
  // Profiling Storage: Stores total nanoseconds spent in each level
  std::vector<long long> layer_times_ns_; 
