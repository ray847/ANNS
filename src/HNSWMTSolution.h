...
private:
  ...
  struct SearchStats {
    ...
    long long total_dist_calcs = 0; // <--- NEW: Track distance calculations
...
public:
  ...
  ~Solution() {
    ...
    double avg_dist_calcs = (double)stats_.total_dist_calcs / stats_.count; // <--- NEW
    ...
    // Print from top layer down to 0
    for (int l = MAX_LEVEL; l >= 0; l--) {
      // Only print layers that actually exist in the built graph to save space
  ...
  void search(const std::vector<float>& query, int* res) {
    ...
    int local_hops = 0;
    long long local_dist_calcs = 0; // <--- NEW: Local counter
    ...
    // --- Update Global Stats ---
    {
      ...
      stats_.total_dist_calcs += local_dist_calcs; // <--- NEW
