...
class Solution {
 public:
  ...
  void search(const std::vector<float>& query, int* res) {
    ...
    auto top = BeamSearchDynamic(q_data, curr_ep, kEfSearch, 0, visited_list_);
 ...
 private:
  ...
  std::priority_queue<std::pair<float, int>> BeamSearchDynamic(
      const float* query_data, int entry_point, int initial_ef, int level,
      VisitedList& visited) {
    
    using QueueItem = std::pair<float, int>;
    std::priority_queue<QueueItem> top_candidates; // Keeps the 'ef' best results found
    std::priority_queue<QueueItem, std::vector<QueueItem>,
                        std::greater<>>
        candidates; // Min-heap: Explores the closest nodes first
    ...
    float best_dist_so_far = initial_dist;
    
    // Dynamic EF parameters
    float current_ef = static_cast<float>(initial_ef);
    const float kDecayRate = 0.97f; // Shrink by 3% on non-improving steps
    const int kMinEf = 20;          // Safety floor to prevent over-shrinking

    while (!candidates.empty()) {
      auto [curr_dist, curr_id] = candidates.top();
      
      // Dynamic Exit Condition:
      // Check against 'current_ef' instead of the fixed 'ef'.
      if (curr_dist > top_candidates.top().first &&
          top_candidates.size() >= static_cast<int>(current_ef)) {
        break;
      }
      candidates.pop();

      // --- Logic: Check if we are improving ---
      // If the node we are processing is worse than our best known, 
      // we assume we might be exploring a less optimal area, so we tighten the beam.
      if (curr_dist > best_dist_so_far) {
          if (current_ef > kMinEf) {
              current_ef *= kDecayRate;
          }
      } else {
          // We found a new best (or equal)!
          // Optional: You could reset current_ef here to encourage more exploration
          // around this new promising node, or just keep it steady.
          best_dist_so_far = curr_dist;
      }
      ...
      for (int i = 0; i < size; ++i) {
        ...
        if (!visited.Visit(neighbor_id)) {
          ...
          // Use 'current_ef' for the admission check
          if (top_candidates.size() < static_cast<int>(current_ef) ||
              d < top_candidates.top().first) {
            candidates.push({d, neighbor_id});
            top_candidates.push({d, neighbor_id});
            
            // Prune top_candidates to stay within the dynamic limit
            if (top_candidates.size() > static_cast<int>(current_ef)) {
                top_candidates.pop();
            }
          }
